c      Fortran version of fAGNSED
c      Ref. Hagen and Done (in prep.)
c
c      This is AGNSED (Kubota & Done 2018), but including a colour
c      temperature correction in the standard disc region
c
...
c-----------------------------------------------------------------------
       ...
       subroutine fagnsed(ear,ne,param,ifl,photar,photer)
       implicit none
...
c      param(13): fcol, colour-temperature correction
...
c      Call model if parameters or energy grid have changed
       if (parchange.or.echange) then
          ...
          call calc_fagnsed(enew, Nnew, param, ifl, ph)
...
c-----------------------------------------------------------------------
       ...
       subroutine calc_fagnsed(es, nn, param, ifl, ph)
...
c       
       ...
       double precision kTh, kTw, fcol, fcol_r
       ...
       real ntwpar(5), nthpar(5)
       real ntwpherr(nn), nthpherr(nn)
...
c-----------------------------------------------------------------------
       if (return_disc) then
          ...
          do i=1, int(nrd), 1
             ...
             tr=nttemp(rmid, M, mdot, Mdot_edd, astar, risco)
